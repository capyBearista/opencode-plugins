...
export async function runReleaseGuard(options: ReleaseGuardOptions): Promise<ReleasePlan> {
  ...
  const exitCode = await runner(args, createChangesetsEnvironment(env), cwd);
  if (exitCode !== 0) {
...
function createChangesetsEnvironment(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  return {
    ...env,
    HUSKY: "0",
...
const runChangesets: ChangesetsRunner = async (args, env, cwd) => {
